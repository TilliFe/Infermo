...
from autograd.utils import shape_a, shape_b, strides_a, strides_b, recursive_broadcast, recursive_broadcast_bw
...
@parameter
fn kernel_add_fw(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
    ...
    fn v_add[nelts: Int](i: Int):
        c.store_data[nelts](
            offset_c + i, a.load_data[nelts](offset_a + i) + b.load_data[nelts](offset_b + i)
        )
...
@parameter
fn kernel_add_bw_a(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
    ...
    fn v_add_grad_a[nelts: Int](i: Int):
        a.store_grad[nelts](
            offset_a + i, a.load_grad[nelts](offset_a + i) + c.load_grad[nelts](offset_c + i)
        )
    vectorize[nelts, v_add_grad_a](c_rest)

@parameter
fn kernel_add_bw_b(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
    ...
    fn v_add_grad_b[nelts: Int](i: Int):
        b.store_grad[nelts](
            offset_b + i, b.load_grad[nelts](offset_b + i) + c.load_grad[nelts](offset_c + i)
        )
...
@parameter
fn kernel_mul_fw(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
...
@parameter
fn kernel_mul_bw_a(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
    ...
    fn v_mul_grad_a[nelts: Int](i: Int):
        a.store_grad[nelts](
            offset_a + i, a.load_grad[nelts](offset_a + i) + b.load_data[nelts](offset_b + i) *  c.load_grad[nelts](offset_c + i)
        )
    vectorize[nelts, v_mul_grad_a](c_rest)

@parameter
fn kernel_mul_bw_b(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
    ...
    fn v_mul_grad_b[nelts: Int](i: Int):
        b.store_grad[nelts](
            offset_b + i, b.load_grad[nelts](offset_b + i) + a.load_data[nelts](offset_a + i) * c.load_grad[nelts](offset_c + i)
        )
...
@parameter
fn kernel_sub_fw(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
...
@parameter
fn kernel_sub_bw_a(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
...
@parameter
fn kernel_sub_bw_b(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
...
@parameter
fn kernel_div_fw(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
...
@parameter
fn kernel_div_bw_a(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
    ...
    fn v_div_grad_a[nelts: Int](i: Int):
        a.store_grad[nelts](
            offset_a + i, a.load_grad[nelts](offset_a + i) + c.load_grad[nelts](offset_c + i) / b.load_data[nelts](offset_b + i)
        )
    vectorize[nelts, v_div_grad_a](c_rest)

@parameter
fn kernel_div_bw_b(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
    ...
    fn v_div_grad_b[nelts: Int](i: Int):
        b.store_grad[nelts](
            offset_b + i, b.load_grad[nelts](offset_b + i) - a.load_data[nelts](offset_a + i) * c.load_grad[nelts](offset_c + i)  / pow(b.load_data[nelts](offset_b + i),2)
        )
...
@parameter
fn kernel_matmul_fw(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
 
    let offset_a = a_index * a.shape_ptr.load().load(a.num_dims_ptr.load()-2) * a.shape_ptr.load().load(a.num_dims_ptr.load()-1)
    let offset_b = b_index * b.shape_ptr.load().load(b.num_dims_ptr.load()-2) * b.shape_ptr.load().load(b.num_dims_ptr.load()-1)
    let offset_c = c_index * c.shape_ptr.load().load(c.num_dims_ptr.load()-2) * c.shape_ptr.load().load(c.num_dims_ptr.load()-1) 
    ...
    fn calc_row_fw(m: Int):
        for k in range(K):
            ...
            fn dot_fw[_nelts: Int](n: Int):
                c.store_data[_nelts](
                    offset_c + m*N+n, 
                    c.load_data[_nelts](offset_c + m*N+n) + a.load_data(offset_a + m*K+k) * b.load_data[_nelts](offset_b + k*N+n))
...
@parameter
fn kernel_matmul_bw_a(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
    ...
    fn calc_row_1(m: Int):
        for n in range(N):
            ...
            fn dot_bw_a[nelts: Int](k: Int):
                let val = a.load_grad(offset_a + m * K + k) + c.load_grad(offset_c + m * N + n) * b.load_data(offset_b + k * N + n) 
...
@parameter
fn kernel_matmul_bw_b(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
    ...
    fn calc_row_2(k: Int):
        for m in range(M):
            ...
            fn dot_bw_b[nelts: Int](n: Int):
                let val = b.load_grad(offset_b + k * N + n) + a.load_data(offset_a + m * K + k) * c.load_grad(offset_c + m * N + n)
...
fn fw_sqrt(node: Node, parent1: Node):
    ...
    fn v_sqrt[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, sqrt(parent1.load_data[_nelts](i))
        )
...
fn bw_sqrt(node: Node, parent1: Node):
    ...
    fn v_sqrt_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + node.load_grad[_nelts](i) / (Float32(2.0) * sqrt(parent1.load_data[_nelts](i)))
        )
...
fn fw_abs(node: Node, parent1: Node):
    ...
    fn v_abs[_nelts: Int](i: Int):
        ...
        node.store_data[_nelts](
            i, (parent1.load_data[_nelts](i) >= zeros).cast[DType.float32]() * parent1.load_data[_nelts](i) + (parent1.load_data[_nelts](i) < zeros).cast[DType.float32]() * (-parent1.load_data[_nelts](i))
        )
...
fn bw_abs(node: Node, parent1: Node):
    ...
    fn v_abs_bw[_nelts: Int](i: Int):
        let zeros = SIMD[DType.float32,_nelts]()
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + (Float32(2.0) * (parent1.load_data[_nelts](i) >= zeros).cast[DType.float32]() - Float32(1.0)) * node.load_grad[_nelts](i)
        )
...
fn fw_exp2(node: Node, parent1: Node):
    ...
    fn v_exp2[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, exp2(parent1.load_data[_nelts](i))
        )
...
fn bw_exp2(node: Node, parent1: Node):
    ...
    fn v_exp2_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + node.load_grad[_nelts](i) * node.load_data[_nelts](i) * log(Float32(2.0))
        )
...
fn fw_exp(node: Node, parent1: Node):
    ...
    fn v_exp[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, exp(parent1.load_data[_nelts](i))
        )
...
fn bw_exp(node: Node, parent1: Node):
    ...
    fn v_exp_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + node.load_grad[_nelts](i) * node.load_data[_nelts](i)
        )
...
fn fw_log2(node: Node, parent1: Node):
    ...
    fn v_log2[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, log2(parent1.load_data[_nelts](i))
        )
...
fn bw_log2(node: Node, parent1: Node):
    ...
    fn v_log2_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + node.load_grad[_nelts](i) / (parent1.load_data[_nelts](i) * log(Float32(2.0)))
        )
...
fn fw_log(node: Node, parent1: Node):
    ...
    fn v_log[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, log(parent1.load_data[_nelts](i))
        )
...
fn bw_log(node: Node, parent1: Node):
    ...
    fn v_log_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + node.load_grad[_nelts](i) / parent1.load_data[_nelts](i)
        )
...
# kernel
@parameter
fn kernel_pow_fw(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
    ...
    fn v_pow[nelts: Int](i: Int):
        c.store_data[nelts](
            offset_c + i, pow(a.load_data[nelts](offset_a + i), b.load_data[nelts](offset_b + i))
        )
...
# kernel
@parameter
fn kernel_pow_bw_a(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
    ...
    fn v_pow_bw_a[nelts: Int](i: Int):
        a.store_grad[nelts](
            offset_a + i, a.load_grad[nelts](offset_a + i) + b.load_data[nelts](offset_b + i) * pow(a.load_data[nelts](offset_a + i),b.load_data[nelts](offset_b + i) - Float32(1.0)) * c.load_grad[nelts](offset_c + i)
        )
    vectorize[nelts, v_pow_bw_a](c_rest)

@parameter
fn kernel_pow_bw_b(c: Node, a: Node, b: Node, a_index: Int, b_index: Int, c_index: Int, depth: Int) -> None:
    ...
    fn v_pow_bw_b[nelts: Int](i: Int):
        b.store_grad[nelts](
            offset_b + i, b.load_grad[nelts](offset_b + i) + c.load_data[nelts](offset_c + i) * log(a.load_data[nelts](offset_a + i)) * c.load_grad[nelts](offset_c + i)
        )
...
fn fw_sin(node: Node, parent1: Node):
    ...
    fn v_sin[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, sin(parent1.load_data[_nelts](i))
        )
...
fn bw_sin(node: Node, parent1: Node):
    ...
    fn v_sin_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + cos(parent1.load_data[_nelts](i)) * node.load_grad[_nelts](i)
        )
...
fn fw_cos(node: Node, parent1: Node):
    ...
    fn v_cos[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, cos(parent1.load_data[_nelts](i))
        )
...
fn bw_cos(node: Node, parent1: Node):
    ...
    fn v_cos_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) - sin(parent1.load_data[_nelts](i)) * node.load_grad[_nelts](i)
        )
...
fn fw_tan(node: Node, parent1: Node):
    ...
    fn v_tan[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, tan(parent1.load_data[_nelts](i))
        )
...
fn bw_tan(node: Node, parent1: Node):
    ...
    fn v_tan_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + node.load_grad[_nelts](i) / pow(cos(parent1.load_data[_nelts](i)),2)
        )
...
fn fw_asin(node: Node, parent1: Node):
    ...
    fn v_asin[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, asin(parent1.load_data[_nelts](i))
        )
...
fn bw_asin(node: Node, parent1: Node):
    ...
    fn v_asin_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + node.load_grad[_nelts](i) / sqrt(Float32(1.0) - pow(parent1.load_data[_nelts](i),2))
        )
...
fn fw_acos(node: Node, parent1: Node):
    ...
    fn v_acos[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, acos(parent1.load_data[_nelts](i))
        )
...
fn fw_atan(node: Node, parent1: Node):
    ...
    fn v_atan[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, atan(parent1.load_data[_nelts](i))
        )
...
fn bw_atan(node: Node, parent1: Node):
    ...
    fn v_atan_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + node.load_grad[_nelts](i) / (Float32(1.0) + pow(parent1.load_data[_nelts](i),2))
        )
...
fn fw_sinh(node: Node, parent1: Node):
    ...
    fn v_sinh[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, sinh(parent1.load_data[_nelts](i))
        )
...
fn bw_sinh(node: Node, parent1: Node):
    ...
    fn v_sinh_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + node.load_grad[_nelts](i) * cosh(parent1.load_data[_nelts](i))
        )
...
fn fw_cosh(node: Node, parent1: Node):
    ...
    fn v_cosh[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, cosh(parent1.load_data[_nelts](i))
        )
...
fn bw_cosh(node: Node, parent1: Node):
    ...
    fn v_cosh_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + node.load_grad[_nelts](i) * sinh(parent1.load_data[_nelts](i))
        )
...
fn fw_tanh(node: Node, parent1: Node):
    ...
    fn v_tanh[_nelts: Int](i: Int):
        node.store_data[_nelts](
            i, tanh(parent1.load_data[_nelts](i))
        )
...
fn bw_tanh(node: Node, parent1: Node):
    ...
    fn v_tanh_bw[_nelts: Int](i: Int):
        parent1.store_grad[_nelts](
            i, parent1.load_grad[_nelts](i) + node.load_grad[_nelts](i) * (Float32(1.0) - pow(node.load_data[_nelts](i),2))
        )
...
fn fw_relu(node: Node, parent1: Node):
    ...
    fn v_relu[_nelts: Int](i: Int):
        let zeros = SIMD[DType.float32,_nelts]()
        node.store_data[_nelts](
            i, abs((parent1.load_data[_nelts](i) > zeros).cast[DType.float32]() * parent1.load_data[_nelts](i))
        )
...
fn bw_relu(node: Node, parent1: Node):
    ...
    fn v_relu_bw[_nelts: Int](i: Int):
        let zeros = SIMD[DType.float32,_nelts]()
        parent1.store_grad[_nelts](
            i, abs((parent1.load_data[_nelts](i) > zeros).cast[DType.float32]()) * node.load_grad[_nelts](i) + parent1.load_grad[_nelts](i)
        )
    vectorize[nelts, v_relu_bw](node.load_cap())

# copy ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# forward
fn fw_copy(node: Node, parent1: Node):  pass

# backward
fn bw_copy(node: Node, parent1: Node): pass
        ...
        fn v_softmax_bw_outer[nelts: Int](j: Int):
            ...
            fn v_softmax_bw[nelts: Int](i: Int):
                if i == j:
                    ...
                    grad += (node.load_grad[nelts](offset + i) * (temp * (Float32(1.0) - temp))).reduce_add()
                else:
                    grad += (node.load_grad[nelts](offset + i) * node.load_data[nelts](offset + i) * node.load_data[nelts](offset + j) * -1).reduce_add()
            vectorize[nelts, v_softmax_bw](N)
            parent1.store_grad[nelts](offset + j, parent1.load_grad[nelts](offset + j) + grad)
...
fn fw_mse(node: Node, parent1: Node, parent2: Node):
    ...
    fn v_mse[nelts: Int](i: Int):
        let error = (parent1.load_data[nelts](i) - parent2.load_data[nelts](i)) * (parent1.load_data[nelts](i) - parent2.load_data[nelts](i))
...
fn bw_mse(node: Node, parent1: Node, parent2: Node):
    ...
    fn v_mse_bw[nelts: Int](i: Int):
        let grad = - Float32(2.0) * (parent2.load_data[nelts](i) - parent1.load_data[nelts](i)) / Float32(parent1.load_cap())
...
fn bw_ce(c: Node, a: Node, b: Node):
    ...
    for index in range(a.cap_ptr.load()):
        ...
        a.store_grad(index,  a.load_grad(index) +  grad_a / (Float32(a.cap_ptr.load()) / Float32(N)))
    for index in range(b.cap_ptr.load()):
        let grad_b = - a.load_data(index) / (b.load_data(index))
        b.store_grad(index,  b.load_grad(index) + grad_b / (Float32(a.cap_ptr.load()) / Float32(N)))
...
fn bw_reshape(node: Node, parent1: Node):
    for s in range(node.cap_ptr.load() // parent1.cap_ptr.load()):
        ...
        @parameter
        fn v_reshape[nelts: Int](i: Int):
            parent1.store_grad[nelts](i, parent1.load_grad[nelts](i) + node.load_grad[nelts](i))
        vectorize[nelts, v_reshape](parent1.cap_ptr.load())
    ...
    for s in range(node.cap_ptr.load() // (M * N)):
        ...
        for i in range(M):
            ...
            fn v_transpose[nelts: Int](j: Int):
                node.store_data[nelts](offset + j * M + i, parent1.load_data[nelts](offset + i * N + j))
        ...
        for i in range(M):
            ...
            fn v_transpose_bw[nelts: Int](j: Int):
                parent1.store_grad[nelts](
                    offset + j * M + i,
                    parent1.load_grad[nelts](offset + j * M + i) + node.load_grad[nelts](offset + i * N + j)
                )
